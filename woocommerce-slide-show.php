...
function woo_slide_show_display( $attributes ){
		...
		if ( $attributes['tag'] !== false ){
			$atag = trim($attributes['tag']);
			$atag = explode( ',', $atag);
			$queryargs['tag']  = $atag;
		}

		if ( $attributes['category'] !== false ){
			$acategory = trim($attributes['category']);
			$acategory = explode( ',', $acategory);
			$queryargs['category']  = $acategory;
		}

		if ( $attributes['include'] !== false ){
			$ainclude = trim( $attributes['include'] );
			$ainclude = explode( ',', $ainclude);
			$ainclude = array_map( function($v){ return (int)$v;}, $ainclude);
			$queryargs['include']  = $ainclude;
		}
	...
	//	return "<pre>" . var_export( $atags, true ) . "</pre>";
		...
		if ( count($product->get_gallery_image_ids()) !== 0  ){
			...
			if ( $attributes['tracking'] !== '' ){
					$link = $product->get_permalink() . $attributes['tracking'];
			} else {
				$link = $product->get_permalink();
			}
		$products[] = array(
...
foreach ( $products as $key => $value ) {
	...
	data-cycle-shuffle-right=0
	    ...
	    data-cycle-timeout=2000 data-woo-link="' . $value['link'] . '">';
	...
	$product_html .= '<a href="' . $cta_url . '" class="woo-slide-cta">' .$cta_text . '</a>';
...
$slideTitle
<div class='cycle-slideshow woo-slide-show-container' data-cycle-fx=flipHorz
    data-cycle-timeout=5000 data-cycle-speed=800 data-cycle-slides='> div.woo-slide-product-div'
...
return woo_slide_show_display( $parsed_atts );
